...
코드를 읽다가 "왜 이렇게 짰지?" 싶은 부분에 대한 의도를 기록합니다.
...
| 확장자 | 사용 기준 | 예시 |
|--------|-----------|------|
| `.tsx` | JSX를 반환하는 컴포넌트 | `PostWritePage.tsx`, `Button.tsx` |
| `.ts` | UI가 없는 순수 로직 | `ai.ts`, `useAIGenerate.ts`, `utils.ts` |

확장자만 보고도 해당 파일이 화면을 그리는지, 로직만 담당하는지 바로 알 수 있습니다.
`.ts` 파일에 실수로 JSX를 넣으면 TypeScript가 컴파일 오류를 발생시켜 역할 혼용을 방지합니다.

### AI 관련 코드를 3개 레이어로 분리한 이유

AI 기능은 다음 3가지 레이어로 나뉩니다.

```
src/shared/
├── api/ai.ts           # 1. 통신 레이어 — API를 어떻게 호출하는가
├── hooks/useAIGenerate.ts  # 2. UX 레이어 — 로딩·덮어쓰기 모달 등 UI 흐름
└── prompts/            # 3. 도메인 레이어 — AI에게 무엇을 물어보는가
    ├── postPrompt.ts
    └── productPrompt.ts
```

각 레이어는 **하나의 이유로만 변경**됩니다.
즉 관심사 분리(Separation of Concerns) 원칙을 따릅니다. 파일이 하나의 이유로만 변경되도록 역할을 나눴습니다.

| 레이어 | 변경 시점 |
|--------|-----------|
| `ai.ts` | API 엔드포인트나 요청 형식이 바뀔 때 |
| `useAIGenerate.ts` | 로딩 UX, 덮어쓰기 확인 흐름이 바뀔 때 |
| `prompts/` | AI 지시문(프롬프트) 내용이 바뀔 때 |

모두 한 파일에 두면 프롬프트 문구 하나 수정하려다 UI 로직을 실수로 건드리는 일이 생깁니다.

### `buildMessages`를 값이 아닌 함수로 받는 이유

```ts
// 이렇게 하면 훅 초기화 시점의 상태가 고정됨
buildMessages: ChatMessage[]

// 이렇게 하면 버튼을 누르는 시점의 최신 상태를 참조함
buildMessages: () => ChatMessage[]
```

훅은 컴포넌트가 **렌더링될 때** 초기화되지만, AI 생성 버튼은 **나중에** 눌립니다.
함수로 받으면 `generate()`가 호출되는 시점에 실행되어 그때의 `images`, `imageUrl` 등 최신 상태를 캡처할 수 있습니다. (지연 평가, Lazy Evaluation)

### API URL을 환경변수로 분리한 이유

API URL을 코드에 직접 작성하면 레포가 공개될 경우 URL이 노출됩니다.
`.env`는 `.gitignore`에 등록되어 레포에 올라가지 않으며, `.env.example`만 공유해 팀원이 직접 값을 채우도록 합니다.
URL이 변경될 때도 코드를 수정하지 않고 환경변수만 교체하면 됩니다.
